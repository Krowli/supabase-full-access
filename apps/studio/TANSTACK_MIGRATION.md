...
- [x] `routes/api/platform/auth/$ref/**` — the five routes stage 1 of this fork added (`config`, `config/hooks`, `templates/$template/content`, `templates/$template/reset`, `validate/spam`) are mirrored and registered in `routeTree.gen.ts`. The tree was regenerated programmatically — `Generator` + `getConfig` from `@tanstack/router-generator`, given `routeTreeFileFooter` from `@tanstack/start-plugin-core`'s `buildRouteTreeFileFooterFromConfig`; without that footer the run deletes the `declare module '@tanstack/react-start'` Register block. Full script and procedure in the fork's local (untracked) `.superpowers/sdd/studio-fork-auth/task-3-report.md`. Stage 2 of the fork added six more mirrors the same way — `routes/api/platform/projects/$ref/config/{pgbouncer,supavisor,realtime,storage}` and `routes/api/platform/storage/$ref/credentials/{index,$id}` — while `routes/api/platform/projects/$ref/config/postgrest` only gained a `PATCH` export, which needs no regeneration because the tree records routes and not methods; `routes/api/platform/database/$ref/pooling` was deleted along with its pages-router handler, and is the one route removed from the tree.
